{
  ...
  nixosModule =
    { config, lib, ... }:
    {
      options.virtualisation.allVmVariants = lib.mkOption { type = lib.types.deferredModule; };
      ...
      config = {
          ...
          { user, config, ... }:
          {
            ...
            home-manager.sharedModules = [
              (
                { lib, ... }:
                {
                  services.polybar.config."bar/centre".monitor = lib.mkVMOverride "Virtual-1";

                  xsession.windowManager.i3.config.workspaceOutputAssign = lib.mkVMOverride [
                    {
                      workspace = "101";
                      output = "Virtual-1";
                    }
                  ...
                  wayland.windowManager.sway.config.workspaceOutputAssign = lib.mkVMOverride [
                    {
                      workspace = "1";
                      output = "Virtual-1";
                    }
            ...
            virtualisation.qemu =
              let
                pkgs' = config.virtualisation.host.pkgs;
              in
              {
                options = lib.mkIf pkgs'.stdenv.hostPlatform.isLinux [
                  "-display gtk,grab-on-hover=true,gl=on"
                  # Use a better fake GPU
                  (lib.mkIf pkgs'.stdenv.hostPlatform.isx86_64 "-vga none -device virtio-vga-gl")
                ];
        ...
        virtualisation.vmVariantWithBootLoader = config.virtualisation.allVmVariants;

{
  darwinModule =
    ...
    {
      environment.systemPackages = builtins.attrValues { inherit (pkgs) syncthing-macos; };

      launchd.user.agents.syncthing = {
        command = ''"/Applications/Nix Apps/Syncthing.app/Contents/MacOS/Syncthing"'';
  ...
  nixosModule =
    ...
    }:
    {
      ...
      services.syncthing.dataDir = lib.mkDefault "${config.users.users.${user}.home}/Sync";
      ...
      services.syncthing.settings =
        let
          ...
          mkFolder =
            ...
            in
            lib.optionalAttrs (devices ? ${hostname}) {
              ${name} = filteredArgs;
            };
        in
        {
          devices = lib.mkMerge (
            map mkDevice [
              {
                name = "hyperion-macos";
                id = "OEAWYP7-XUL5L4A-WH4PM4P-UNTIIXC-3C2LWFY-F2WPKNY-H4TFMWI-IFZSCQU";
              }
              {
                name = "phi-nixos";
                id = "2YEN2S7-JYISWE4-UGUF6N4-7ZNSDNX-IKLEDGT-4WLFFGV-CWB2VKG-SL3ALAP";
              }
              {
                name = "sigma";
                id = "IU52G3V-Z5YCNHH-R7WEYEI-BPDXUMH-7T374SL-LRN5TZW-WAMOJL7-SK5LFQV";
              }
              {
                name = "gaia";
                id = "GMBP5QR-5VCPMTG-FBMIZK3-WXJXU2F-75P5QHZ-6KKOKEE-HLHMQLX-IJJPPAP";
              }
            ]
          );
            ...
            map mkFolder [
              {
                ...
                devices = {
                  ...
                  sigma = {
                    path = "/persist${config.users.users.${user}.home}/Pictures/Pictures.sigma";
                  };
                };
              }
              {
                ...
                devices = {
                  ...
                  sigma = {
                    path = "/persist${config.users.users.${user}.home}/Pictures/Screenshots.sigma";
                  };
                };
              }
              {
      ...
      systemd.services.syncthing-init = {
        serviceConfig.LoadCredential = "gui-password:${config.clan.core.vars.generators.syncthing.files.password.path}";
      };
    };
}

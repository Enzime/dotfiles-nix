{
  ...
  homeModule =
    ...
    {
      home.packages = builtins.attrValues { inherit (pkgs) xclip fira-mono font-awesome_5; };
      ...
      xsession.windowManager.i3.config = {
        ...
        keybindings =
          ...
          in
          {
            ...
            "Control+Mod4+${mod}+q" = "--release exec ${lib.getExe pkgs.xorg.xkill}";
            ...
            "${mod}+Shift+Return" = "exec ${lib.getExe pkgs.alacritty} -o 'window.class.instance=\"floating\"'";
      ...
      services.polybar.config = {
        ...
        "module/dotfiles" = {
          ...
          exec = "${pkgs.writeShellScript "latest-dotfiles" ''
            ...
            LATEST=$(${lib.getExe pkgs.curl} -s https://github.com/Enzime/dotfiles-nix/commit/HEAD.patch | ${lib.getExe' pkgs.coreutils "head"} -n 1 | ${lib.getExe' pkgs.coreutils "cut"} -d ' ' -f 2)
            ...
            if ! ${lib.getExe pkgs.git} merge-base --is-ancestor $LATEST ''${RUNNING%-dirty} 2>/dev/null; then
              UPDATE_FOUND=true
            fi
      ...
      systemd.user.services.xss-lock =
        ...
        lib.mkIf config.services.screen-locker.enable (
          lib.mkForce {

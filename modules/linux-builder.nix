      ...
      config = {
        ...
        nix.linux-builder.config =
          { lib, ... }:
          {
            ...
            users.users.root.openssh.authorizedKeys.keys = builtins.attrValues { inherit (keys.users) enzime; };
        ...
        nix.settings.trusted-public-keys = [ keys.signing."${host}-linux-builder" ];
      };
    };
}

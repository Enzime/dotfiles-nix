  ...
  nixosModule = { options, config, pkgs, lib, ... }: {
    ...
    boot.initrd.network.hoopsnake = {
      ...
      systemd-credentials = {
        ...
        clientId.file =
          config.clan.core.vars.generators.hoopsnake.files.tailscale-client-id.path;
        clientId.encrypted = false;
        clientSecret.file =
          config.clan.core.vars.generators.hoopsnake.files.tailscale-client-secret.path;

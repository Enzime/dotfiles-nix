    ...
    imports = [{
      config = lib.optionalAttrs (options ? clan) {
        ...
        clan.core.vars.generators.readeck-user = {
          ...
          files.password-hash = { };
          runtimeInputs = [ pkgs.coreutils pkgs.xkcdpass pkgs.openssl ];
          script = ''
    ...
    services.nginx.virtualHosts."readeck.enzim.ee" = {
      ...
      locations."/" = {
        proxyPass = "http://localhost:${
            toString config.services.readeck.settings.server.port
          }";
      };
    };
    ...
    systemd.services.readeck-user = {
      ...
      serviceConfig = {
        ...
        inherit (config.systemd.services.readeck.serviceConfig)
          StateDirectory WorkingDirectory DynamicUser;
        LoadCredential =
          "password-hash:${config.clan.core.vars.generators.readeck-user.files.password-hash.path}";
        ExecStart = pkgs.writeShellScript "readeck-user" ''
          ${
            lib.getExe config.services.readeck.package
          } user -u Enzime -p "$(cat "$CREDENTIALS_DIRECTORY"/password-hash)"
        '';
      };
    };

  ...
  config = {
    perSystem =
      ...
      }:
      {
        ...
        packages.tf = pkgs.opentofu.withPlugins (
          p:
          builtins.attrValues {
            inherit (p)
              valodim_desec
        ...
        terraformConfigurations.everything = inputs.terranix.lib.terranixConfiguration {
          inherit system;
          modules = [ self.terranixModules.everything ];
          extraArgs = { inherit self' inputs inputs'; };
        };

        terraformConfigurations.terragrunt = (pkgs.formats.json { }).generate "terragrunt.hcl.json" {
          remote_state = {
            backend = "s3";

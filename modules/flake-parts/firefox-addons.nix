...
{
  flake = {
    overlays.firefox-addons = final: prev:
      let
        ...
        addons = import
          "${inputs.firefox-addons.sourceInfo.outPath}/pkgs/firefox-addons" {
          ...
          if addons ? ${name} then
            ...
            addon) {
              masked-email-manager = buildFirefoxXpiAddon {
                ...
                url =
                  "https://addons.mozilla.org/firefox/downloads/file/4585287/masked_email_manager-1.7.2.xpi";

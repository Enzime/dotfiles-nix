...
let
  ...
  regularOverlays = filterAttrs (name: _: hasSuffix ".nix" name) (readDir (pathTo ./overlays));
  importedRegularOverlays = mapAttrsToList importOverlay regularOverlays;

  modules' = mapAttrs' (
    filename: _: nameValuePair (removeSuffix ".nix" filename) (importFrom (pathTo ./modules) filename)
  ...
  getModuleList =
    ...
    in
    if (imports == [ ]) then [ a ] else [ a ] ++ unique (concatMap getModuleList imports);
  ...
  mkConfiguration =
    ...
    let
      ...
      homeModules = map (getAttr "homeModule") (filter (hasAttr "homeModule") modulesToImport);
    ...
    in
    {
      ...
      flake.checks.${system} =
        (optionalAttrs nixos {
          "nixos-${hostname}" = self.nixosConfigurations.${hostname}.config.system.build.toplevel;
        })
        // (optionalAttrs isDarwin {
          "nix-darwin-${hostname}" = self.darwinConfigurations.${hostname}.config.system.build.toplevel;
        })
        // {
      ...
      flake.terranixModules =
        optionalAttrs (builtins.pathExists (pathTo ./hosts/${host}/terraform-configuration.nix))
          {
            ${hostname}.imports = [
              self.terranixModules.base
              (inputs.flake-parts.lib.importApply (pathTo ./hosts/${host}/terraform-configuration.nix) {
